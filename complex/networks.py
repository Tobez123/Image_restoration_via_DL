...
class Generator(nn.Module):
    def __init__(self, input_nc=8, norm=nn.InstanceNorm2d, frames_num=15, ndf=16, num_resblock=9, down_sample=2,
                 learn_residual=True):
        ...
        model = [nn.ReflectionPad2d(3), nn.Conv2d(ndf*4, ndf*8, kernel_size=7), norm(ndf*8), nn.ReLU(True)]
        for i in range(down_sample):
            model += [nn.Conv2d(ndf*8*2**i, ndf*8*2**(i+1), kernel_size=3, stride=2, padding=1), norm(ndf*8*2**(i+1)),
                      nn.ReLU(True)]
        for j in range(num_resblock):
            model += [Resblock(ndf*8*2**down_sample, norm)]
        for k in range(down_sample):
            model += [nn.ConvTranspose2d(int(ndf*8*2**(down_sample)*(1/2)**(k)), int(ndf*8*2**down_sample*(1/2)**(k+1))
                                         , kernel_size=3, stride=2, padding=1, output_padding=1),
                      norm(int(ndf*8*2**down_sample*(1/2)**(k+1))), nn.ReLU(True)]
